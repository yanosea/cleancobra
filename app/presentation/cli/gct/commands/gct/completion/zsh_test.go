...
func TestNewZshCompletionCommand(t *testing.T) {
	tests := []struct {
		name string
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := NewZshCompletionCommand(cobraProxy, rootCmd)
...
func TestRunZshCompletion(t *testing.T) {
	tests := []struct {
		name      string
	...
	}{
		{
			...
			setupMock: func(mockCmd *proxy.MockCommand) {
				mockCmd.EXPECT().GenZshCompletion(gomock.Any()).Return(nil)
			},
			wantErr: false,
		},
		{
			...
			setupMock: func(mockCmd *proxy.MockCommand) {
				mockCmd.EXPECT().GenZshCompletion(gomock.Any()).Return(errors.New("generation failed"))
			},
			wantErr: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := runZshCompletion(mockRootCmd)
