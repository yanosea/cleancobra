...
func NewTodoRepository(conf *config.TodoConfig) (todoDomain.TodoRepository, error) {
	xdgDataHome, err := util.GetXDGDataHome()
	if err != nil {
		return nil, err
	}
	dbFileDirPath := strings.Replace(conf.DBDirPath, "XDG_DATA_HOME", xdgDataHome, 1)
	if err := util.MkdirIfNotExist(dbFileDirPath); err != nil {
		return nil, err
	}
	dbFilePath := filepath.Join(dbFileDirPath, dbFileName)
	if _, err := os.Stat(dbFilePath); os.IsNotExist(err) {
		if err := util.InitializeJSONFile(dbFilePath, []*todoDomain.Todo{}); err != nil {
			return nil, err
		}
	}
